...
local function ResetUnitAuras(unitID)
	if unitID and lib.trackAuras  == true then
...
--~ 	name, rank, icon, count, debuffType, duration, expirationTime, unitCaster, isStealable, shouldConsolidate, spellId  = UnitAura("unit", index or "name"[, "rank"[, "filter"]]) 
	...
	if lib.trackAuras == true and lib.GUIDAuras[dstGUID] then
		for i=#lib.GUIDAuras[dstGUID], 1, -1 do
            local data = lib.GUIDAuras[dstGUID][i]
            if (filterType == "HARMFUL" and data.isDebuff) or (filterType == "HELPFUL" and not data.isDebuff) then
...
--~ 					local spellName = GetSpellInfo(spellID)
    ...
    if change then
        lib.callbacks:Fire("LibAuraInfo_AURA_UPDATE_REAL", dstGUID, filterType)
    end
end
...
-- Implementing a polling mechanism to update auras with real data
-- since UNIT_AURA doesn't fire for targettarget, arenaXtarget or mouseover
local unitPoll = {}
local THROTTLE = 1.000
lib.frame:SetScript("OnUpdate", function(self, elapsed)
	for unit, sinceLast in pairs(unitPoll) do
        if UnitExists(unit) then
            local sinceLast = sinceLast and sinceLast + elapsed or elapsed
            if sinceLast > THROTTLE then
                sinceLast = sinceLast - THROTTLE
                
                unitPoll[unit] = sinceLast
...
function lib.frame:UNIT_TARGET(event, unitID)
...
--~ 	self:checkIfAuraAlreadyOnGUID(dstGUID, spellID, srcGUID)
    ...
    table_insert(self.GUIDAuras[dstGUID], #self.GUIDAuras[dstGUID] + 1, aura)
...
function lib.frame:SPELL_AURA_REMOVED(event, timestamp, eventType, srcGUID, srcName, srcFlags, dstGUID, dstName, dstFlags, ...)	--
    ...
    if lib.auraInfo[spellID] then
        ...
        local isDebuff = auraType == "DEBUFF" and true or false
        local hasAura, count, _, duration, expirationTime = lib:GUIDAuraID(dstGUID, spellID, srcGUID)
        expirationTime = expirationTime and expirationTime or nil
        ...
        lib.callbacks:Fire("LibAuraInfo_AURA_REMOVED", dstGUID, spellID, srcGUID, spellSchool, auraType, isDebuff, duration, expirationTime, count)
    end
end
...
function lib:GUIDAura(dstGUID, i)
	if self.GUIDAuras[dstGUID] and self.GUIDAuras[dstGUID][i] then
		...
		return true, data.stackCount or 0, data.debuffType, data.duration, data.expirationTime, data.isDebuff, data.srcGUID, data.spellID
	end
	return false
end
